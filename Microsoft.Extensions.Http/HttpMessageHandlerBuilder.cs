    ...
    public abstract class HttpMessageHandlerBuilder
    {
        ...
        protected internal static HttpMessageHandler CreateHandlerPipeline(HttpMessageHandler primaryHandler, IEnumerable<DelegatingHandler> additionalHandlers)
        {
            ...
            for (var i = additionalHandlersList.Count - 1; i >= 0; i--)
            {
                ...
                if (handler == null)
                {
                    var message = $"The '{nameof(additionalHandlers)}' must not contain a null entry."; 
                ...
                {
                    var message = $"The '{nameof(DelegatingHandler.InnerHandler)}' property must be null. '{nameof(DelegatingHandler)}' instances provided to '{nameof(HttpMessageHandlerBuilder)}' must not be reused or cached.{Environment.NewLine}Handler: '{handler}'";
                    throw new InvalidOperationException(message);
                }
