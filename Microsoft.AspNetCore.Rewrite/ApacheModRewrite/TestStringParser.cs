    ...
    internal class TestStringParser
    {
            ...
            while (context.Next())
            {
                switch (context.Current)
                {
                    case Percent:
                        ...
                        if (!context.Next())
                        {
                            throw new FormatException($"Unrecognized parameter type: '{testString}', terminated at string index: '{context.Index}'");
                        }
                    ...
                    case Dollar:
                        ...
                        {
                            throw new FormatException($"Cannot parse '{testString}' to integer at string index: '{context.Index}'");
                        }
                        break;
                    default:
        ...
        private static void ParseConditionParameter(ParserContext context, IList<PatternSegment> results)
        {
            ...
            else
            {
                ...
                throw new FormatException($"Cannot parse '{context.Template}' to integer at string index: '{context.Index}'");
            }
        }
