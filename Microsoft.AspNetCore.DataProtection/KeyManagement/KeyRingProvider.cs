        ...
        private CacheableKeyRing CreateCacheableKeyRingCore(DateTimeOffset now, IKey keyJustAdded)
        {
            ...
            if (!_keyManagementOptions.AutoGenerateKeys)
            {
                ...
                if (keyToUse == null)
                {
                    ...
                    throw new InvalidOperationException("The key ring does not contain a valid default protection key. The data protection system cannot create a new key because auto-generation of keys is disabled.");
                }
                else
                {
