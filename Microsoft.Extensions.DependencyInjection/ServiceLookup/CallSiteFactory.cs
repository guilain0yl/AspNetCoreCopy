        ...
        private void Populate()
        {
            foreach (var descriptor in _descriptors)
            {
                ...
                if (serviceTypeInfo.IsGenericTypeDefinition)
                {
                    ...
                    {
                        throw new ArgumentException($"Open generic service type '{descriptor.ServiceType}' requires registering an open generic implementation type.",
                            "descriptors");
                    }

                    if (implementationTypeInfo.IsAbstract || implementationTypeInfo.IsInterface)
                    {
                        throw new ArgumentException($"Cannot instantiate implementation type '{descriptor.ImplementationType}' for service type '{descriptor.ServiceType}'.");
                    }
                }
                else if (descriptor.ImplementationInstance == null && descriptor.ImplementationFactory == null)
                {
                    ...
                    {
                        throw new ArgumentException($"Unable to resolve service for type '{descriptor.ImplementationType}' while attempting to activate '{descriptor.ServiceType}'.");
                    }
                }
                        ...
                        else
                        {
                            ...
                            if (!bestConstructorParameterTypes.IsSupersetOf(parameters.Select(p => p.ParameterType)))
                            {
                                ...
                                var message = string.Join(
                                    Environment.NewLine,$"Unable to activate type '{implementationType}'. The following constructors are ambiguous:",
                                    bestConstructor,
                                    constructors[i]);
