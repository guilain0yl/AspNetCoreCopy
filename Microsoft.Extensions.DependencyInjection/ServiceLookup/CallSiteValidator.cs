    ...
    internal class CallSiteValidator : CallSiteVisitor<CallSiteValidator.CallSiteValidatorState, Type>
    {
        ...
        public void ValidateResolution(Type serviceType, IServiceScope scope, IServiceScope rootScope)
        {
                ...
                && _scopedServices.TryGetValue(serviceType, out var scopedService))
            {
                ...
                {
                    throw new InvalidOperationException($"Cannot resolve {nameof(ServiceLifetime.Scoped).ToLowerInvariant()} service '{serviceType}' from root provider.");
                }

                throw new InvalidOperationException($"Cannot resolve '{serviceType}' from root provider because it requires {nameof(ServiceLifetime.Scoped).ToLowerInvariant()} service '{scopedService}'.");
            }
        }
        ...
        protected override Type VisitScopeCache(ServiceCallSite scopedCallSite, CallSiteValidatorState state)
        {
            ...
            {
                throw new InvalidOperationException($"Cannot consume {nameof(ServiceLifetime.Scoped).ToLowerInvariant()} service '{scopedCallSite.ServiceType}' from {nameof(ServiceLifetime.Singleton).ToLowerInvariant()} '{state.Singleton.ServiceType}'.");
            }
