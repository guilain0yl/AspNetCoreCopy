...
namespace Microsoft.AspNetCore.Routing.Patterns
{
    ...
    public static class RoutePatternFactory
    {
            ...
            IEnumerable<RoutePatternPathSegment> segments)
        {
            ...
            if (parameterPolicies != null && parameterPolicies.Count > 0)
            {
                ...
                foreach (var kvp in parameterPolicies)
                {
                    ...
                    {
                        throw new InvalidOperationException($"Invalid constraint '{kvp.Value ?? "null"}'. A constraint must be of type 'string' or '{typeof(IRouteConstraint)}'.");
                    }
            ...
            RoutePatternPart VisitPart(RoutePatternPart part)
            {
                ...
                if (updatedDefaults != null && updatedDefaults.TryGetValue(parameter.Name, out var newDefault))
                {
                    ...
                    {
                        var message = $"The route parameter '{parameter.Name}' has both an inline default value and an explicit default value specified. A route parameter cannot contain an inline default value when a default value is specified explicitly. Consider removing one of them."; 
        ...
        public static RoutePatternLiteralPart LiteralPart(string content)
        {
            ...
            {
                throw new ArgumentException($"The literal section '{content}' is invalid. Literal sections cannot contain the '?' character.");
            }
        ...
        public static RoutePatternParameterPart ParameterPart(string parameterName)
        {
            ...
            {
                throw new ArgumentException($"The route parameter name '{parameterName}' is invalid. Route parameter names must be non-empty and cannot contain these characters: '{{', '}}', '/'. The '?' character marks a parameter as optional, and can occur only at the end of the parameter. The '*' character marks a parameter as catch-all, and can occur only at the start of the parameter.");
            }
        ...
        public static RoutePatternParameterPolicyReference Constraint(object constraint)
        {
            ...
            {
                throw new InvalidOperationException($"Invalid constraint '{constraint ?? "null"}'. A constraint must be of type 'string' or '{typeof(IRouteConstraint)}'.");
            }
        }
