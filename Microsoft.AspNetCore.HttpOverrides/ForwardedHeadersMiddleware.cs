...
namespace Microsoft.AspNetCore.HttpOverrides
{
        ...
        public void ApplyForwarders(HttpContext context)
        {
            ...
            for (; entriesConsumed < sets.Length; entriesConsumed++)
            {
                ...
                if (checkFor)
                {
                    ...
                    if (___Package.TryParse(set.IpAndPortText, out var parsedEndPoint))
                    {
                        applyChanges = true;
    ...
    public static class ___Package
    {
        public static bool TryParse(string str, out IPEndPoint dst)
        {
            dst = null;
            ...
            if(!IPAddress.TryParse(res[0], out tmp))
            {
                return false;
            }

            dst = new IPEndPoint(tmp, int.Parse(res[1]));
